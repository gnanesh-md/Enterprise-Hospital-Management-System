...
function buildSymptomsSummary(
  ...
  if (latest) {
    ...
    if (latest.bp_systolic && latest.bp_diastolic)
      vitalsParts.push(`BP: ${latest.bp_systolic}/${latest.bp_diastolic}`)
    if (latest.spo2 != null) vitalsParts.push(`SpO2: ${latest.spo2}%`)
    if (latest.temperature != null)
      vitalsParts.push(`Temp: ${latest.temperature}C`)
  }
  const vitalsText =
...
async function fetchAiTriageSuggestion(
  ...
  return apiFetch<AiTriageSuggestion>("/api/symptom-ai/triage", {
    method: "POST",
    body: JSON.stringify({
      symptoms,
...
export default function ErPage({
  ...
  const awaitingDoctorCount =
    (summary["registered"] || 0) + (summary["triaged"] || 0)
                ...
                <Table>
                  ...
                  {visits.map((v) => (
                    <TableRow
                      ...
                      style={{
                        boxShadow: `inset 4px 0 0 0 ${triageColorFor(v.triage_category, categories)}`,
                      }}
                    >
                      ...
                      <TableCell>
                        {v.is_unknown_patient ? (
                          <span>
                            ...
                            />
                            <span className="er-queue-patient-name">
                          ...
                          <div>
                            <span
                              className="er-queue-patient-name"
...
function QuickIntakePanel({
  ...
  const submit = async () => {
    ...
    if (patientMode === "new" && !newName.trim()) {
      setNotice({
        type: "error",
        message: "Patient name is required for ER registration.",
      })
      return
    }
    if (patientMode === "unknown" && !unknownLabel.trim()) {
      setNotice({
        type: "error",
        message:
          'Describe the unknown patient (e.g. "Unidentified male, approx 35-40").',
      })
      return
    }
    ...
    try {
      ...
      if (vitalsBpSys.trim() && vitalsBpDia.trim())
        vitalsParts.push(`BP: ${vitalsBpSys}/${vitalsBpDia}`)
      ...
      if (symptomsText.trim()) {
        try {
          ...
          const categoryCode = mapUrgencyToTriageCategory(
            aiRes.urgency,
            categories,
          )
          if (categoryCode) {
            await apiFetch(`/api/er/visits/${visitId}/triage`, {
              ...
              body: JSON.stringify({
                ...
                reason: (aiRes.reasoning || "AI Triage applied").substring(
                  0,
                  200,
                ),
              }),
  ...
  return (
    <div className="er-intake-panel">
      ...
      <div style={{ marginBottom: "1.2rem" }}>
        ...
        <div
          className="er-mode-grid"
          style={{ marginBottom: "1rem", marginTop: "0.5rem" }}
        >
          <button
            type="button"
            className={`er-mode-card${
              patientMode === "new" ? " er-mode-card-active" : ""
            }`}
          ...
          <button
            type="button"
            className={`er-mode-card${
              patientMode === "existing" ? " er-mode-card-active" : ""
            }`}
          ...
          <button
            type="button"
            className={`er-mode-card${
              patientMode === "unknown" ? " er-mode-card-active" : ""
            }`}
        ...
        {patientMode === "new" && (
          <div
            className="er-direct-registration-form"
          ...
          >
            ...
            <div style={{ display: "flex", gap: "0.5rem" }}>
              ...
              <div style={{ flex: 1 }}>
                <Label
                  htmlFor="er-new-allergies"
                  style={{ fontSize: "0.8rem", color: "#b45309" }}
                >
                  Known Allergies
                </Label>
                <Input
        ...
        {patientMode === "unknown" && (
          <div>
            <Label htmlFor="er-unknown-label">
      ...
      <div style={{ marginBottom: "1.2rem" }}>
        ...
        <div>
          <Label style={{ fontSize: "0.8rem", color: "#64748b" }}>
            Chief Complaints / Trauma Details
          </Label>
          <Textarea
            rows={2}
      ...
      <div style={{ marginTop: "auto" }}>
        ...
        >
          {saving ? (
            ...
            <>
              <FiUserPlus aria-hidden style={{ marginRight: "0.4rem" }} />{" "}
              Register & Admit to ER
            </>
          ) : patientMode === "unknown" ? (
            <>
              <FiHelpCircle aria-hidden style={{ marginRight: "0.4rem" }} />{" "}
              Admit Unidentified Patient
            </>
          ) : (
            <>
              <FiPlus aria-hidden style={{ marginRight: "0.4rem" }} /> Admit
              Patient to ER
            </>
          )}
        </Button>
...
function TriageConfigPanel({
  ...
  const submit = async () => {
    if (!code.trim() || !label.trim()) {
      setNotice({
        type: "error",
        message: "Category code and label are required.",
      })
      return
    }
    setSaving(true)
      ...
      {categories.length === 0 ? (
        <div className="module-empty-state">
          <p className="module-empty-state-title">
            No triage categories configured
          </p>
          <p className="module-empty-state-hint">
            Triage can't be used until your hospital's clinical team defines its
            own priority categories (e.g. B1-B4) below — nothing is pre-filled
            on purpose.
          </p>
        </div>
      ) : (
...
function VisitDetailPanel({
  ...
  const pendingBedRequest = detail.bed_requests.find(
    (r) => r.status === "pending",
  )
  const closedOrNoBedNeeded =
    detail.disposition &&
    !OUTCOMES_REQUIRING_BED.has(detail.disposition.outcome)
    ...
    <section className="module-page">
      ...
      >
        <Button
          variant="ghost"
          onClick={onBack}
          className="er-visit-header-back"
        >
        ...
        <div>
          ...
          <p className="er-visit-sub">
            {detail.is_unknown_patient && (
              <FiHelpCircle aria-hidden style={{ marginRight: "0.3rem" }} />
            )}
            {patientLabel} &middot; Arrived{" "}
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem" }}>
        <button
          ...
          className={`btn btn-sm ${
            viewTab === "clinical" ? "btn-primary" : "btn-secondary"
          }`}
        ...
        >
          <FiActivity aria-hidden style={{ marginRight: "0.3rem" }} /> Clinical
          Workflow & Care
        </button>
        <button
          type="button"
          className={`btn btn-sm ${
            viewTab === "timeline" ? "btn-primary" : "btn-secondary"
          }`}
      ...
      {viewTab === "timeline" ? (
        <div className="panel">
          <SectionHead
            ...
            title="Chronological Emergency Event Timeline (Hosp AI)"
          />
          ...
          <div className="er-detail-layout">
                      ...
                      <>
                        ...
                        >
                          {detail.patient.gender && (
                            <span>
                              Gender: <strong>{detail.patient.gender}</strong>
                            </span>
                          )}
                          {detail.patient.age && (
                            <span>
                              Age: <strong>{detail.patient.age}y</strong>
                            </span>
                          )}
                          {detail.patient.blood_group && (
                            <span>
                              Blood:{" "}
                              <strong>{detail.patient.blood_group}</strong>
                            </span>
                          )}
                        </div>
                        ...
                        {detail.patient.emergency_contact && (
                          <div style={{ color: "#dc2626" }}>
                            Emergency Contact:{" "}
                            <strong>{detail.patient.emergency_contact}</strong>
                          </div>
                        )}
              ...
              <div className="panel">
                ...
                {detail.triage ? (
                  <p>
                    ...
                    >
                      {detail.triage.reason} &middot;{" "}
                      {formatDateTimeIST(detail.triage.triaged_at)}
                    </span>
                  </p>
                ) : (
            ...
            <div className="er-detail-main">
              ...
              <div className="panel">
                  ...
                  {detail.assigned_doctor_name ? (
                    <>
                      ...
                      {detail.doctor_accepted_at ? (
                        <span
                          className="er-status-badge er-status-resolved"
                        ...
                        <span
                          className="er-status-badge er-status-pending"
                          style={{ marginLeft: "0.5rem" }}
                        >
                          Pending accept
                        </span>
                ...
                {detail.assigned_doctor_name && !detail.doctor_accepted_at && (
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await apiFetch(`/api/er/visits/${detail.id}/accept`, {
                          method: "POST",
                        })
                        onRefresh()
                      } catch (error: any) {
                        reportError(
                          setNotice,
                          error,
                          "Failed to accept the assignment.",
                        )
                      }
                    }}
                  >
              ...
              <div className="panel">
                  ...
                  <p>
                    <strong>
                      {OUTCOME_OPTIONS.find(
                        (o) => o.value === detail.disposition!.outcome,
                      )?.label || detail.disposition.outcome}
                    </strong>
                    <br />
                ...
                {pendingBedRequest && (
                  <p className="muted">
                    Bed request sent to Bed Management (
                    {pendingBedRequest.requested_level_of_care.toUpperCase()}) —
                    awaiting allocation.
                  </p>
                )}
...
function ErTimelineView({
  ...
  if (detail.arrival_at) {
    events.push({
      ...
      subtitle: `Arrival Mode: ${detail.arrival_mode || "Walk-in"}${
        detail.condition_at_arrival
          ? ` · Condition: ${detail.condition_at_arrival}`
          : ""
      }`,
      badge: "Arrival",
      type: "arrival",
  ...
  if (detail.triage) {
    const cat = categories.find(
      (c) => c.category_code === detail.triage!.category,
    )
    events.push({
  ...
  if (detail.disposition) {
    events.push({
      ...
      subtitle: `Reason: ${detail.disposition.clinical_reason}${
        detail.disposition.decided_by
          ? ` · Decided by: ${detail.disposition.decided_by}`
          : ""
      }`,
      badge: "Disposition",
      type: "disposition",
      ...
      >
        ...
        >
          <div>
            <h2 style={{ margin: 0, fontSize: "1.2rem", fontWeight: 800 }}>
              HOSP AI EMERGENCY DEPARTMENT
            </h2>
            <div className="muted" style={{ fontSize: "0.8rem" }}>
              Phase 1 Clinical Handover & Transfer Summary (Section 36)
            </div>
          </div>
        ...
        >
          ...
          <div>
            ...
            {detail.assigned_doctor_name
              ? `Dr. ${detail.assigned_doctor_name} (${detail.assigned_specialty})`
              : "ER Covering Staff"}
          </div>
        </div>
    ...
    try {
      ...
      if (filled.length === 0) {
        setNotice({
          type: "warning",
          message:
            "AI couldn't confidently suggest anything from what's charted so far -- fill in the sections below manually.",
        })
      }
    } catch (error: any) {
    ...
    <div className="panel er-ai-panel">
      ...
      {!hasChartedData && (
        <p className="muted er-ai-panel-hint">
          Record a chief complaint or vitals first -- there's nothing for the AI
          to reason from yet.
        </p>
      )}
      ...
      {lastRun && (
        <div className="er-ai-result">
          <p className="er-ai-reasoning">
            <strong>Reasoning:</strong> {lastRun.reasoning}
          </p>
          {lastRun.filled.length > 0 ? (
            <p className="er-ai-filled-note">
              <FiCheck aria-hidden /> Pre-filled into{" "}
              <strong>{lastRun.filled.join(", ")}</strong> below — review each
              one and confirm (or change it) with that section's own button.
            </p>
          ) : (
            <p className="muted" style={{ margin: 0 }}>
              Nothing confident enough to suggest yet -- use the sections below
              manually.
            </p>
          )}
        </div>
    ...
    <ul className="er-list">
      {treatments.map((t) => (
        <li key={t.id}>
          ...
          <span className="muted">
            {" "}
            &middot; {formatDateTimeIST(t.performed_at)}
          </span>
        </li>
      ))}
    </ul>
    ...
    try {
      ...
      if (!aiRes.department && !aiRes.doctor) {
        setNotice({
          type: "warning",
          message:
            "AI couldn't suggest a department/doctor from what's charted so far -- select one manually below.",
        })
      }
    } catch (error: any) {
  ...
  return (
    <div style={{ marginTop: "0.75rem" }}>
      ...
      >
        <FiZap aria-hidden />{" "}
        {suggesting
          ? "Analyzing complaints & vitals..."
          : "Suggest from complaints & vitals"}
      </Button>
      {suggestionReason && (
        <p
          className="muted"
