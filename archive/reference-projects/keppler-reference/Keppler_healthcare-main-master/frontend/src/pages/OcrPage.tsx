    ...
    <section className="page-section ocr-scanner-page">
      <Tabs>
        <TabsTrigger active={tab === "upload"} onClick={() => setTab("upload")}>
          <span className="ocr-tab-label">
            <Icon name="upload" size={15} /> Upload &amp; Scan
          </span>
        </TabsTrigger>
        <TabsTrigger active={tab === "vault"} onClick={openVaultTab}>
          <span className="ocr-tab-label">
            <Icon name="folder" size={15} /> My Documents
            {vaultDocs.length > 0 ? ` (${vaultDocs.length})` : ""}
          </span>
        </TabsTrigger>
        <TabsTrigger active={tab === "chat"} onClick={openChatTab}>
          <span className="ocr-tab-label">
            <Icon name="chat" size={15} /> Ask AI
          </span>
        </TabsTrigger>
      </Tabs>
          ...
          <div className="ocr-workspace-grid">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Choose a document type, then upload an image or PDF for AI
                  text extraction.
                </CardDescription>
              </CardHeader>
            ...
            <Card className="ocr-status-card">
              <CardHeader>
                ...
                <CardDescription>
                  Live progress for your current scan.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {!jobStatus && (
                  <div className="ocr-empty-panel">
                    ...
                    <p className="muted">
                      Upload a document to see extraction progress here.
                    </p>
                  </div>
                )}
        ...
        <TabsContent>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Everything you've previously scanned. Click a row to view the
                full text.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {vaultDocs.length === 0 && (
                <div className="ocr-empty-panel">
                  ...
                  <p className="muted">
                    No documents yet. Scan one from the Upload &amp; Scan tab.
                  </p>
                </div>
              )}
              {vaultDocs.length > 0 && (
                    ...
                    <TableRow key={doc.id}>
                      ...
                      <TableCell>
                        {doc.confidence_score != null ? (
                          <Badge
                            variant={confidenceVariant(doc.confidence_score)}
                          >
                      ...
                      <TableCell>
                        <div className="ocr-row-actions">
                          <Button
                            ...
                            size="sm"
                            type="button"
                            onClick={() => void handleAddToKnowledgeBase(doc)}
                          >
                            Add to KB
                          </Button>
                          <Button
          ...
          <div className="ocr-chat-grid">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Documents currently available to Ask AI.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {kbDocs.length === 0 && (
                  <div className="ocr-empty-panel ocr-empty-panel-compact">
                    <p className="muted">
                      Add a document from My Documents to start chatting.
                    </p>
                  </div>
                )}
                <div className="ocr-kb-list">
                  {kbDocs.map((doc) => (
                    <div key={doc.doc_id} className="ocr-kb-item">
                      <span className="ocr-kb-item-name">
                        <Icon name="file" size={14} /> {doc.filename}
                      </span>
                      <Button
                        variant="ghost"
                        size="sm"
            ...
            <Card className="ocr-chat-card">
              <CardHeader className="ocr-chat-header">
                <div>
                  ...
                  <CardDescription>
                    Ask questions grounded in your knowledge base documents.
                  </CardDescription>
                </div>
                <Button
                  variant="ghost"
              ...
              <CardContent className="ocr-chat-content">
                <div className="ocr-chat-history">
                  ...
                  {chatMessages.map((message, index) => (
                    <div
                      key={index}
                      className={`ocr-chat-message ocr-chat-${message.role}`}
                    >
                      <span className="ocr-chat-role">
                  ...
                  {chatLoading && (
                    <p className="muted ocr-chat-thinking">Thinking...</p>
                  )}
                </div>
